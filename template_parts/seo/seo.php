<?php 
// удалим стандартный вывод title и вызовем свой
...
function kama_render_seo_tags(){
	...
	echo kama_meta_description();
	echo kama_meta_keywords();
		...
		if( $is_post ){
			...
			if( ! $thumb_id = get_post_thumbnail_id( $post ) ){
				...
				if( ! $thumb_id = $fn__get_thumb_id_from_text( $post->post_content ) ) {
					...
					$attach = get_children( 'numberposts=1&post_mime_type=image&post_type=attachment&post_parent='. $post->ID );
 ...
 * @version 2.2
 */
function kama_meta_description( $home_description = '', $maxchar = 260 ){
	static $cache; if( $cache ) return $cache;
